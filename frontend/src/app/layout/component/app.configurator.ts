...
export class AppConfigurator {
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        const preset = presets[this.layoutService.layoutConfig().preset as PresetName];
        const presetPalette = preset.primitive as Record<string, SurfacePalette> | undefined;
    ...
    updateColors(event: Event, type: ColorType, color: SurfacesType): void {
        if (type === 'primary') {
    ...
    applyTheme(type: ColorType, color: SurfacesType): void {
        if (type === 'primary') {
    ...
    onPresetChange(value: string): void {
        this.layoutService.layoutConfig.update((state) => ({ ...state, preset: value }));
        const preset = presets[value as PresetName];
