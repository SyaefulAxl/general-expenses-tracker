...
/**
 * Sakai augments PrimeNG's MenuItem with a `path` segment (used to track the
 * active hierarchy) and an optional `badgeClass`. Typed explicitly so the
 * template stays free of `any`.
 */
...
export class AppMenuitem {
    ...
    item = input<LayoutMenuItem | null>(null);
    ...
    updateActiveStateFromRoute() {
        ...
        const routerLink = item?.routerLink as string[] | undefined;
        if (!routerLink) return;

        const isRouteActive = this.router.isActive(routerLink[0], {
            paths: 'exact',
            queryParams: 'ignored',
